...
namespace Data.Mapping
{
    public class PlansSubscriptionMap : IEntityTypeConfiguration<PlansSubscription>
    {
        public void Configure(EntityTypeBuilder<PlansSubscription> builder)
        {
            ...
            builder.HasOne(x => x.Plan).WithMany(x => x.PlansSubscriptions).HasForeignKey(x => x.PlansId);
            builder.HasOne(x => x.User).WithMany(x => x.PlansSubscriptions).HasForeignKey(x => x.UserId);
        }
    }
}
